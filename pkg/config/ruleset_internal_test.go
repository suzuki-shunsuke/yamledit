...
func TestReadRulesets(t *testing.T) { //nolint:funlen,maintidx
	t.Parallel()
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tt.want, got); diff != "" {
				t.Errorf("ReadRulesets() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
...
func TestReadRulesetsByPaths_remoteURL(t *testing.T) {
	t.Parallel()
	remoteConfig := testRemoteConfig
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Write([]byte(remoteConfig)) //nolint:errcheck
	...
	dir := t.TempDir()
	configs, err := ReadRulesetsByPaths(context.Background(), slog.Default(), nil, nil, dir, []string{srv.URL + "/migration.yaml"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(configs) != 1 {
	...
	if diff := cmp.Diff(want, configs[0].Rules); diff != "" {
		t.Errorf("ReadRulesetsByPaths() mismatch (-want +got):\n%s", diff)
	}
}

func TestReadRulesetsByPaths_localPathEscape(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestReadRulesets_withReusableRules(t *testing.T) {
	t.Parallel()
	remoteConfig := `rules:
  - path: "$"
    actions:
...
func TestReadRulesetsByPaths_reusableRuleFallback(t *testing.T) {
	t.Parallel()
	remoteConfig := testRemoteConfig
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Write([]byte(remoteConfig)) //nolint:errcheck
...
func TestReadRulesetsByPaths_localOverReusableRule(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestReadRulesetsByPaths_globalConfigFallback(t *testing.T) {
	remoteConfig := testRemoteConfig
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Write([]byte(remoteConfig)) //nolint:errcheck
	}))
