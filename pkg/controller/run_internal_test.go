...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := Run(context.Background(), newLogger(), nil, dir, nil, []string{yamlFile})
			if tt.wantErr {
				if err == nil {
...
`)
	...
	if err := Run(context.Background(), newLogger(), nil, dir, nil, []string{file1, file2}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
`)
		...
		if err := Run(context.Background(), newLogger(), nil, dir, nil, []string{yamlFile}); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		got, err := os.ReadFile(yamlFile)
		if err != nil {
...
`)
	...
	err := Run(context.Background(), newLogger(), nil, dir, nil, []string{filepath.Join(dir, "nonexistent.yaml")})
	if err == nil {
